...
class ESPLoader(object):
    ...
    def detect_chip(port=DEFAULT_PORT, baud=ESP_ROM_BAUD, connect_mode='default_reset'):
        ...
        print_wx_info(Console_wx, 'Detecting chip type...')
        sys.stdout.flush()
        ...
        for cls in [ESP8266ROM, ESP32ROM]:
            if date_reg == cls.DATE_REG_VALUE:
                ...
                print_wx_info(Console_wx, ' %s' % inst.CHIP_NAME)
                return inst
        print_wx_info(Console_wx, '')
    ...
    def connect(self, mode='default_reset'):
        ...
        print_wx_info(Console_wx, 'Connecting...')
        sys.stdout.flush()
        last_error = None
        ...
        try:
            ...
            print_wx_info(Console_wx, '')  # end 'Connecting...' line
    ...
    def flash_begin(self, size, offset):
        ...
        if size != 0 and not self.IS_STUB:
            print_wx_info(Console_wx, "Took %.2fs to erase flash block" % (time.time() - t))
        self._port.timeout = DEFAULT_TIMEOUT
        return num_blocks
    ...
    def run_stub(self, stub=None):
        ...
        # Upload
        print_wx_info(Console_wx, "Uploading stub...")
        ...
        print_wx_info(Console_wx, "Running stub...")
        self.mem_finish(stub['entry'])

        p = self.read()
        if p != b'OHAI':
            raise FatalError("Failed to start stub. Unexpected response: %s" % p)
        print_wx_info(Console_wx, "Stub running...")
    ...
    def flash_defl_begin(self, size, compsize, offset):
        ...
        print_wx_info(Console_wx, "Compressed %d bytes to %d..." % (size, compsize))
    ...
    def change_baud(self, baud):
        print_wx_info(Console_wx, "Changing baud rate to %d" % baud)
    ...
    def warn_if_unusual_segment(self, offset, size, is_irom_segment):
        if not is_irom_segment:
            if offset > 0x40200000 or offset < 0x3ffe0000 or size > 65536:
                print_wx_info(Console_wx, 'WARNING: Suspicious segment 0x%x, length %d' % (offset, size))
    ...
    def __init__(self, load_file=None):
        ...
        if load_file is not None:
            ...
            if segments != ESPBOOTLOADER.IMAGE_V2_SEGMENT:
                ...
                print_wx_info(Console_wx, 'Warning: V2 header has unexpected "segment" count %d (usually 4)' % segments)
            ...
            if first_flash_mode != self.flash_mode:
                print_wx_info(Console_wx, 'WARNING: Flash mode value in first header (0x%02x) disagrees with second (0x%02x). Using second value.'
                      % (first_flash_mode, self.flash_mode))
            if first_flash_size_freq != self.flash_size_freq:
                print_wx_info(Console_wx, 'WARNING: Flash size/freq value in first header (0x%02x) disagrees with second (0x%02x). Using second value.'
                      % (first_flash_size_freq, self.flash_size_freq))
            if first_entrypoint != self.entrypoint:
                print_wx_info(Console_wx, 'WARNING: Entrypoint address in first header (0x%08x) disagrees with second header (0x%08x). Using second value.'
                      % (first_entrypoint, self.entrypoint))
    ...
    def __init__(self, load_file=None):
        ...
        if load_file is not None:
            ...
            if additional_header != self.EXTENDED_HEADER:
                print_wx_info(Console_wx, "WARNING: ESP32 image header contains unknown flags. Possibly this image is from a different version of esptool.py")
...
class ELFFile(object):
    ...
    def _read_sections(self, f, section_header_offs, section_header_count, shstrndx):
        ...
        if sec_type != ELFFile.SEC_TYPE_STRTAB:
            print_wx_info(Console_wx, 'WARNING: ELF file has incorrect STRTAB section type 0x%02x' % sec_type)
        f.seek(sec_offs)
...
def load_ram(esp, args):
    ...
    print_wx_info(Console_wx, 'RAM boot...')
    for (offset, size, data) in image.segments:
        print_wx_info(Console_wx, 'Downloading %d bytes at %08x...' % (size, offset), end=' ')
        sys.stdout.flush()
...
def dump_mem(esp, args):
    ...
    for i in range(args.size // 4):
        ...
        if f.tell() % 1024 == 0:
            print_wx_info(Console_wx, '\r%d bytes read... (%d %%)' % (f.tell(),
...
def detect_flash_size(esp, args):
    if args.flash_size == 'detect':
        ...
        if args.flash_size is None:
            print_wx_info(Console_wx, 'Warning: Could not auto-detect Flash size (FlashID=0x%x, SizeID=0x%x), defaulting to 4MB' % (flash_id, size_id))
            args.flash_size = '4MB'
        else:
...
def write_flash(esp, args):
    ...
    for address, argfile in args.addr_filename:
        if args.no_stub:
            print_wx_info(Console_wx, 'Erasing flash...')
        ...
        while len(image) > 0:
            print_wx_info(Console_wx, '\rWriting at 0x%08x... (%d %%)' % (address + seq * esp.FLASH_WRITE_SIZE, 100 * (seq + 1) // blocks))
        ...
        if args.compress:
            ...
            print_wx_info(Console_wx, '\rWrote %d bytes (%d compressed) at 0x%08x in %.1f seconds%s...' % (uncsize, written, address, t, speed_msg))
        else:
            if t > 0.0:
                speed_msg = " (%.1f kbit/s)" % (written / t * 8 / 1000)
            print_wx_info(Console_wx, '\rWrote %d bytes at 0x%08x in %.1f seconds%s...' % (written, address, t, speed_msg))
        try:
            res = esp.flash_md5sum(address, uncsize)
            if res != calcmd5:
                print_wx_info(Console_wx, 'File  md5: %s' % calcmd5)
                print_wx_info(Console_wx, 'Flash md5: %s' % res)
                print_wx_info(Console_wx, 'MD5 of 0xFF is %s' % (hashlib.md5(b'\xFF' * uncsize).hexdigest()))
                raise FatalError("MD5 of file does not match data in flash!")
            else:
                print_wx_info(Console_wx, 'Hash of data verified.')
    ...
    if args.verify:
        print_wx_info(Console_wx, 'Verifying just-written flash...')
...
def image_info(args):
    ...
    print_wx_info(Console_wx, 'Entry point: %08x' % image.entrypoint if image.entrypoint != 0 else 'Entry point not set')
    print_wx_info(Console_wx, '%d segments' % len(image.segments))
    print_wx_info
    idx = 0
    for seg in image.segments:
        idx += 1
        print_wx_info(Console_wx, 'Segment %d: %r' % (idx, seg))
...
def elf2image(args):
    ...
    if args.chip == 'auto':  # Default to ESP8266 for backwards compatibility
        print_wx_info(Console_wx, "Creating image for ESP8266...")
        args.chip == 'esp8266'
...
def read_mac(esp, args):
    ...
    def print_wx_info_mac(label, mac):
...
def flash_id(esp, args):
    flash_id = esp.flash_id()
    print_wx_info(Console_wx, 'Manufacturer: %02x' % (flash_id & 0xff))
...
def read_flash(esp, args):
    ...
    t = time.time() - t
    print_wx_info(Console_wx, '\rRead %d bytes at 0x%x in %.1f seconds (%.1f kbit/s)...'
...
def verify_flash(esp, args):
    ...
    for address, argfile in args.addr_filename:
        ...
        print_wx_info(Console_wx, 'Verifying 0x%x (%d) bytes @ 0x%08x in flash against %s...' % (image_size, image_size, address, argfile.name))
        # Try digest first, only read if there are differences.
        digest = esp.flash_md5sum(address, image_size)
        expected_digest = hashlib.md5(image).hexdigest()
        if digest == expected_digest:
            print_wx_info(Console_wx, '-- verify OK (digest matched)')
            continue
        else:
            differences = True
            if getattr(args, 'diff', 'no') != 'yes':
                print_wx_info(Console_wx, '-- verify FAILED (digest mismatch)')
                continue

        flash = esp.read_flash(address, image_size)
        assert flash != image
        diff = [i for i in range(image_size) if flash[i] != image[i]]
        print_wx_info(Console_wx, '-- verify FAILED: %d differences, first @ 0x%08x' % (len(diff), address + diff[0]))
        for d in diff:
            flash_byte = flash[d]
            image_byte = image[d]
            if PYTHON2:
            ...
            print_wx_info(Console_wx, '   %08x %02x %02x' % (address + d, flash_byte, image_byte))
    if differences:
        raise FatalError("Verify failed.")


def read_flash_status(esp, args):
    print_wx_info(Console_wx, 'Status value: 0x%04x' % esp.read_status(args.bytes))


def write_flash_status(esp, args):
    fmt = "0x%%0%dx" % (args.bytes * 2)
    args.value = args.value & ((1 << (args.bytes * 8)) - 1)
    print_wx_info(Console_wx, ('Initial flash status: ' + fmt) % esp.read_status(args.bytes))
    print_wx_info(Console_wx, ('Setting flash status: ' + fmt) % args.value)
    esp.write_status(args.value, args.bytes, args.non_volatile)
    print_wx_info(Console_wx, ('After flash status:   ' + fmt) % esp.read_status(args.bytes))


def version(args):
    print_wx_info(Console_wx, __version__)
...
def Burn(console, board,filename,port,is_erase,writeFlashAddr):
    args=Args()
    global Console_wx
    Console_wx = console
    sys.stdout = console
    ...
    if board=='esp32':
      ...
      args.chip='auto'
    print_wx_info(Console_wx, '---------------------%s'%filename)
    if is_erase == "no":
      ...
      args.port=port
      print_wx_info(Console_wx, args.addr_filename)
      print_wx_info(Console_wx, '---------------------')
      print_wx_info(Console_wx, 'esptool.py v%s' % __version__)
      sys.stdout.flush()
    else:
      chip='auto'
      args.port=port
    ...
    if operation_args[0] == 'esp':  # operation function takes an ESPLoader connection object
        ...
        if args.baud > initial_baud:
            try:
                ...
                print_wx_info(Console_wx, "WARNING: ROM doesn't support changing baud rate. Keeping initial baud rate %d" % initial_baud)
                sys.stdout.flush()
        # override common SPI flash parameter stuff as required
        if hasattr(args, "ucIsHspi"):
            print_wx_info(Console_wx, "Attaching SPI flash...")
            sys.stdout.flush()
            esp.flash_spi_attach(args.ucIsHspi,args.ucIsLegacy)
        else:
            esp.flash_spi_attach(0, 0)
        if hasattr(args, "flash_size"):
            print_wx_info(Console_wx, "Configuring flash size...")
            sys.stdout.flush()
        ...
        if args.after == 'hard_reset':
            print_wx_info(Console_wx, 'Hard resetting...')
            sys.stdout.flush()
            esp.hard_reset()
        elif args.after == 'soft_reset':
            print_wx_info(Console_wx, 'Soft resetting...')
            sys.stdout.flush()
            # flash_finish will trigger a soft reset
            esp.soft_reset(False)
        else:
            print_wx_info(Console_wx, 'Staying in bootloader.')
            sys.stdout.flush()
            if esp.IS_STUB:
        ...
        if is_erase == "no":
            try:
                ...
                print_wx_info(Console_wx, "ndm-----------------2-")
                print_wx_info(Console_wx, e)
                print_wx_info(Console_wx, "ndm-----------------1-")
    else:
        operation_func(args)
...
def expand_file_arguments():
    ...
    for arg in sys.argv:
        if arg.startswith("@"):
            ...
            new_args.append(arg)
    if expanded:
        print_wx_info(Console_wx, "esptool.py %s" % (" ".join(new_args[1:])))
        sys.argv = new_args
...
class FlashSizeAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        try:
            ...
            print_wx_info(Console_wx, "WARNING: Flash size arguments in megabits like '%s' are deprecated." % (values[0]))
            print_wx_info(Console_wx, "Please use the equivalent size '%s'." % (value))
            print_wx_info(Console_wx, "Megabit arguments may be removed in a future release.")
        except KeyError:
            value = values[0]
